...
export interface ShareButtonsProps {
  ...
  platforms?: Platform[];
  /** Display variant */
  variant?: "icons" | "buttons" | "floating";
...
> = {
  ...
  copy: {
    ...
    bgColor: "bg-background-gray-light hover:bg-border",
    getUrl: () => "",
  },
};
...
export function ShareButtons({
  ...
  if (variant === "floating") {
    return (
      <div
        className={cn(
          "fixed left-4 top-1/2 z-40 hidden -translate-y-1/2 flex-col gap-2 lg:flex",
          className
        )}
      >
        {platforms.map((platform) => {
          ...
          return (
            ...
            >
              {platform === "copy" && copied ? <CheckIcon /> : config.icon}
            </button>
          );
        })}
      </div>
